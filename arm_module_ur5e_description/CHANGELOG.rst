^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Changelog for package arm_module_ur5e_description
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
* arm_module_ur5e_description: Make robot name configurable on load_ur.launch (`#612 <https://github.com/ros-industrial/universal_robot/issues/612>`_)
...
* fixing deprecation warning
* arm_module_ur5e_description: Use the per-model xacro files for correct robot name (`#588 <https://github.com/ros-industrial/universal_robot/issues/588>`_)
...
* Update arm_module_ur5e_description/urdf/ur_macro.xacro
...
* removed --inorder for common launch
  * default in melodic
* update arm_module_ur5e_description launch files
...
* made common macro for ur_robot urdf
  ...
  * remved arm_module_gazebo specific parts
* create parameters yaml files
  * one for each models
* deleted ur_e specific repositories
  * moved ur_e_description meshes files to arm_module_ur5e_description
